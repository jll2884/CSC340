    ...
    */
    ...
    public int do_lock(IORB iorb)
    {
    	...
    	if(!isValid()) {
    		...
    		if(getValidatingThread()!=null && getValidatingThread() == thread) {
    			getFrame().incrementLockCount();
    			return SUCCESS;
    		}
    		
    		else if(getValidatingThread()!=thread && getValidatingThread()!=null) {
    			thread.suspend(this);
    		...
    		else if(getValidatingThread() == null){
				if (PageFaultHandler.do_handlePageFault(thread, MemoryLock, this)==FAILURE){
					return FAILURE;
				}
	    	}
